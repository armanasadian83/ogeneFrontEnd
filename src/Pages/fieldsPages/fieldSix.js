...
const FieldSix = () => {
    ...
    useEffect(() => {
        fetchDataFromApi(`/api/service?filterKey=بالینی و مدل حیوانی`).then((res) => {
            setServiceData(res);
        });
        fetchDataFromApi(`/api/course?filterKey=بالینی و مدل حیوانی`).then((res) => {
            setCourseData(res);
        })
    }, []);

    const fieldName = "بالینی و مدل حیوانی";
    ...
    return (
        ...
        <div className="feildCellularAndMolecular">
            ...
            <FieldsBanner name={'بالینی و مدل حیوانی / هیستولوژی'} ref={ref} />
