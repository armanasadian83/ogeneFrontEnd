...
const FieldThree = () => {
    ...
    useEffect(() => {
        fetchDataFromApi(`/api/service?filterKey=نانوفناوری`).then((res) => {
            setServiceData(res);
        });
        fetchDataFromApi(`/api/course?filterKey=نانوفناوری`).then((res) => {
            setCourseData(res);
        })
    }, []);

    const fieldName = "نانوفناوری";

    return (
        <>
        <div className="feildCellularAndMolecular">
            
            <FieldsBanner name={'نانوفناوری'} ref={ref} />
